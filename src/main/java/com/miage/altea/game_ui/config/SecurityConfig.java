...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public UserDetailsService userDetailsService() {
        return username -> Optional.ofNullable(trainerService.getTrainer(username))
                .map(trainerPokemonTypeWithLevel -> User.withUsername(trainerPokemonTypeWithLevel.getTrainer().getName()).password(trainerPokemonTypeWithLevel.getTrainer().getPassword()).roles("USER").build())
